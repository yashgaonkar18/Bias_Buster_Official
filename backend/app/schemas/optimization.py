...
class OptimizationResponse(BaseModel):
    ...
    optimization_id: Optional[str] = Field(
        None, description="Unique ID for this optimization run"
    )
    baseline_model: Optional[Dict[str, Any]] = Field(
        None, description="Metrics for original model"
    )
    optimized_model: Optional[Dict[str, Any]] = Field(
        None, description="Metrics for optimized model"
    )
    improvements: Optional[Dict[str, Any]] = Field(
        None, description="Calculated improvements"
    )
    best_params: Dict[str, Any] = Field(
        default_factory=dict, description="Best hyperparameters found"
    )
    ...
    optimized_model_available: bool = Field(
        default=False, description="Whether optimized model was saved"
    )
    download_endpoint: Optional[str] = Field(
        None, description="Endpoint to download optimized model"
    )
    optimization_summary: str = Field(
        default="", description="Summary of optimization results"
    )

    # Original backward compatibility / trial details
    trials_run: int = Field(default=0, description="Number of trials executed")
    comparison: List[TrialResult] = Field(
        default_factory=list, description="Top trials for comparison"
    )
