...
from app.utils import fairness_metrics
...
from app.schemas.retraining import RetrainingRequest

ARTIFACTS_ROOT_DIR = Path(getattr(settings, "ARTIFACT_DIR", "exports"))
...
def run_retraining(
    ...
    if strategy == "reweighting":
        ...
        try:
            mitigated_model, _, _, _ = apply_smote(
                X_base_train,
                y_base_train,
                sensitive_train,
                uploaded_model,
    ...
    if strategy == "smote":
        ...
        try:
            retrained_uploaded, _, _, _ = apply_smote(
...
async def execute_retraining_pipeline(payload: RetrainingRequest, session: AsyncSession):
    # Fetch upload record
    record = (
    ...
    result = run_retraining(
        ...
        train_additional_models=payload.train_additional_models,
