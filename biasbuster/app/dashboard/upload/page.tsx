    ...
    return (
      <div className="max-w-5xl space-y-6">
        <div className="border-b pb-4">
          <h3 className="font-phase text-gray-800 text-lg">
            Dataset and Model Configuration
          </h3>
          <p className="text-xs text-gray-500 mt-1">Configure your experiment parameters and upload files.</p>
        </div>

        {/* Workspace & Experiment Info Block */}
        <div className="bg-white border rounded-lg p-5 shadow-sm">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Current Workspace</span>
              <span className="text-sm font-semibold text-gray-700 bg-gray-50 border border-gray-200 px-3 py-1.5 rounded block">
                {selectedWorkspace?.name || "None Selected"}
              </span>
            </div>
            <div>
              <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block mb-1">Current Experiment</span>
              <span className="text-sm font-semibold text-orange-600 bg-orange-50/50 border border-orange-200 px-3 py-1.5 rounded block font-mono">
                {selectedExperiment?.name || "None Selected"}
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div
            ...
            className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-orange-400 hover:bg-orange-50 transition-all cursor-pointer h-full flex flex-col justify-center min-h-[160px]"
          >
            ...
            {datasetFile && (
              <div className="text-xs text-gray-600 mt-3 font-semibold truncate bg-white border px-2 py-1 rounded">
          ...
          <div
            ...
            className="border-2 border-dashed border-gray-300 rounded-lg p-6 text-center hover:border-blue-400 hover:bg-blue-50 transition-all cursor-pointer h-full flex flex-col justify-center min-h-[160px]"
          >
            ...
            <p className="text-xs text-gray-500">
              Click to browse (PKL or Joblib files)
            </p>
        ...
        <div className="pt-2">
          <button
            onClick={uploadDatasetAndModel}
            disabled={uploading || !datasetFile || !modelFile}
            className="w-full py-3 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 text-white rounded-lg font-bold flex items-center justify-center gap-2 shadow transition-all active:scale-[0.99] cursor-pointer"
          >
            <Upload className="w-4 h-4" />
            {uploading ? "Uploading files..." : "Upload and Validate"}
          </button>
        </div>

        {uploadError && (
          <div className="text-sm text-red-600 mt-4 bg-red-50 p-3 rounded-lg border border-red-100 font-medium">
            {uploadError}
          </div>
        )}
      </div>
        ...
        <div className="flex items-center justify-between p-4 bg-blue-50 border border-blue-200 rounded-lg">
          ...
          <button
            onClick={() => {
              ...
              router.push("/dashboard/bias");
            }}
            className="font-phase px-6 py-2 bg-orange-500 text-white rounded hover:bg-orange-600 flex items-center gap-2 transition-all active:scale-95 shadow-sm cursor-pointer"
            style={{ fontSize: '14px' }}
          >
            Go to Detection
