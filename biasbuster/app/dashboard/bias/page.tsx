  ...
  if (!uploadId) {
    return (
      <div className="p-12 text-center text-gray-500">
        Please upload a dataset and model in the Upload phase first.
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl space-y-6">
        <div className="border-b pb-4">
          <h3 className="font-phase text-gray-800 text-lg">Bias Detection Configuration</h3>
          <p className="text-xs text-gray-500 mt-1">Configure Target Columns and Sensitive Attributes for audit.</p>
        </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 gap-2">
            {columns.map((col) => (
              <label
                key={col}
                className={`flex items-center gap-2 text-sm border rounded px-3 py-2 cursor-pointer transition-all ${
                  selectedSensitive.includes(col) ? "bg-orange-50 border-orange-500" : "hover:bg-gray-50 bg-white"
                }`}
              >
                <input
                  type="checkbox"
        ...
        <div className="pt-2">
          <button
            ...
            disabled={!selectedTarget || selectedSensitive.length === 0}
            className="w-full py-3 bg-orange-500 hover:bg-orange-600 disabled:bg-gray-300 text-white rounded-lg font-bold flex items-center justify-center gap-2 shadow transition-all active:scale-[0.99] cursor-pointer font-phase"
            style={{ fontSize: "14px" }}
          >
            Run Bias Detection
          </button>
    ...
    return (
      <div className="space-y-6 animate-in fade-in slide-in-from-bottom-4 duration-700">
        <div className="p-4 border rounded-lg bg-gray-50 text-sm mb-4">
          <div className="font-semibold mb-2 text-gray-700">Fairness Metric Guidelines</div>
          <ul className="space-y-1 text-gray-600">
            <li>DPD &le; 0.1 &rarr; Fair</li>
            <li>EOD &le; 0.1 &rarr; Fair</li>
            <li>DIR between 0.8 and 1.25 &rarr; Fair</li>
          </ul>
        </div>
        ...
        <div className={`p-4 rounded-lg border flex items-center justify-between gap-3 ${
          biasResults.bias_present ? "bg-red-50 border-red-200 text-red-700" : "bg-green-50 border-green-200 text-green-700"
        }`}
        >
          ...
          {biasResults.bias_present && (
            <button
              onClick={async () => {
                ...
                router.push("/dashboard/mitigation");
              }}
              className="font-phase px-6 py-2 bg-orange-500 text-white rounded hover:bg-orange-600 flex items-center gap-2 shadow-sm transition-all active:scale-95 cursor-pointer text-xs"
            >
              Go to Mitigation
