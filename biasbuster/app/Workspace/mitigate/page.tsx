...
export default function MitigatePage() {
    ...
    return (
      <div className="max-w-5xl space-y-8 animate-in fade-in duration-500">
        <div className="border-b pb-4">
          <h3 className="font-phase">Strategy Recommendation</h3>
        ...
        <div className="border-2 border-blue-200 bg-blue-50 rounded-lg p-6 relative overflow-hidden">
          ...
          <h4 className="text-lg font-bold text-blue-900 mb-2 capitalize flex items-center gap-2">
            <Brain className="w-5 h-5" /> {recommendation?.recommended_strategy}
          </h4>
        ...
        {!isExperimentMode ? (
          <div className="flex flex-col items-center justify-center p-8 border border-dashed border-gray-300 rounded-lg bg-gray-50 mt-6">
            <h4 className="font-phase mb-2 text-gray-700" style={{ fontSize: '18px' }}>Want to explore alternative mitigations?</h4>
            ...
            <button
              ...
              className="px-6 py-2 bg-white border border-gray-300 text-gray-700 rounded font-semibold hover:bg-gray-100 flex items-center gap-2 shadow-sm transition-all active:scale-95"
            >
              <Settings className="w-4 h-4" /> Experiment with Other Strategies
            </button>
          </div>
        ) : (
          <div className="mt-8 space-y-6">
            <h4 className="font-phase mb-4 flex items-center gap-2">
              <Settings className="w-5 h-5" /> Ranked Strategies (Experiment Mode)
            </h4>
            {recommendation?.strategy_rankings?.map((strat: any) => (
              <div
                key={strat.strategy}
                className={`p-4 border rounded-lg cursor-pointer transition-all ${selectedStrategy === strat.strategy
              ...
              >
                <div className="flex justify-between items-start mb-2">
                  <div className="font-bold capitalize flex items-center gap-2">
                    {strat.recommended && <span className="text-xs bg-yellow-100 text-yellow-800 px-2 py-0.5 rounded">Recommended</span>}
                    <span className="text-gray-500">#{strat.rank}</span> {strat.strategy}
                  </div>
                  <div className="text-sm font-semibold text-gray-600">Score: {strat.combined_score.toFixed(3)}</div>
                </div>
                <p className="text-sm text-gray-600 mb-3">{strat.tradeoff_analysis}</p>
                <div className="flex gap-4 text-xs mt-3 pt-3 border-t">
                  ...
                  <div><span className="text-gray-500">Stability:</span> <span className="font-semibold">{(strat.stability_score * 100).toFixed(1)}%</span></div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="flex items-center gap-4 pt-6 border-t mt-8">
            ...
            className="font-phase px-6 py-2 bg-blue-600 text-white rounded hover:bg-blue-700 flex items-center gap-2 shadow-sm transition-all active:scale-95"
            style={{ fontSize: '16px' }}
          >
            <CheckCircle className="w-5 h-5" /> Apply Recommended Strategy
          </button>
          {isExperimentMode && (
            <button
              onClick={() => applyMitigation(selectedStrategy)}
              className="px-6 py-2 bg-white border border-gray-300 text-gray-700 rounded font-semibold hover:bg-gray-50 shadow-sm transition-all active:scale-95"
            >
  ...
  const renderResponseBody = () => {
    ...
    // Helper for robust metric extraction from backend results
                ...
                <div className="grid grid-cols-3 gap-3">
                    {[
                        ...
                        <div key={item.label} className="rounded-lg border border-orange-100 bg-white/80 p-3">
                            ...
                            <div className="text-[11px] font-semibold">Acc: {typeof item.accuracy === "number" ? item.accuracy.toFixed(3) : "-"}</div>
                            <div className="text-[11px] font-semibold">Fair: {typeof item.fairness === "number" ? item.fairness.toFixed(3) : "-"}</div>
                        </div>
                    ))}
                </div>
        ...
        <div className="flex items-center justify-between pt-4">
          <div className="flex gap-3">
            ...
            <button
              ...
              className="px-4 py-2 bg-white border border-gray-300 text-gray-700 rounded-lg text-sm font-semibold hover:bg-gray-50 flex items-center gap-2 shadow-sm transition-all active:scale-95"
            >
              <BarChart3 className="w-4 h-4" /> View Comparison Dashboard
            </button>
          </div>
          <button
            onClick={() => {
                setRequestMethod("OPTIMIZE");
                router.push("/Workspace/optimize");
            }}
            className="font-phase px-6 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700 flex items-center gap-2 shadow-sm transition-all active:scale-95"
            style={{ fontSize: '16px' }}
          >
            <Settings className="w-5 h-5" /> Proceed to Optimization
          </button>
        </div>
      </div>
    );
