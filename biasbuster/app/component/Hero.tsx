...
import { ChevronRight, Cpu, Shield, Database, Play, Pause, Volume2, VolumeX } from "lucide-react";
    ...
    <section className="relative min-h-screen pt-32 pb-20 overflow-hidden flex flex-col justify-center items-center">
      ...
      <div className="container mx-auto px-4 flex flex-col items-center text-center z-10">
        ...
        <h1 className="max-w-4xl text-[64px] md:text-8xl font-anton  uppercase leading-[0.9] tracking-tighter text-foreground  mb-6">
          High-Performance <br />
          Fairness For <br />
          <span className="relative inline-block">AI Models</span>
        </h1>

        <p className="max-w-2xl text-lg md:text-xl text-muted-foreground mb-10 font-medium leading-relaxed">
          Detect, analyze, and mitigate bias in machine learning models with
          automated fairness metrics, intelligent mitigation strategies, and
        ...
        >
          <div className="w-full h-full rounded-xl overflow-hidden relative">
            ...
            <div className="absolute inset-0 z-20 flex flex-col justify-end p-6 opacity-0 group-hover:opacity-100 transition-opacity duration-300">
              <div className="flex items-center gap-4 bg-background/80 backdrop-blur-md p-3 rounded-2xl w-fit border border-border shadow-lg mx-auto sm:mx-0">
                <button
                  onClick={togglePlay}
                  className="p-3 hover:bg-foreground/10 rounded-xl transition-colors text-foreground flex items-center justify-center focus:outline-none focus:ring-2 focus:ring-primary"
            ...
            <video
              ...
              preload="auto"
            >
              <source src="/videos/biasbuster_demo.mp4" type="video/mp4" />
            </video>
          </div>
        </div>
      </div>
